"""Case study: 16 DOF = 4 x 4-DOF.

Serial mass-spring chain with non-uniform properties, split into 4 subsystems.
...
Two equivalent routes are shown: the Python API and the YAML twin 08_case_16dof.yaml.
Usage: python 08_case_16dof.py [--central] [--no-save]      (CI_T overrides the 20 s horizon)
"""
...
if __name__ == "__main__":
    ...
    if not args.no_save:
        save_figures(res, "case16_" + SCHEDULE, PARTITION, unknowns)

    # ---- YAML twin: identical problem, no Python beyond ci.solve ---------------------------------
    if T == 20.0:
        compare_with_yaml(res, os.path.join(HERE, "08_case_16dof.yaml"), unknowns)
